...
// Serves an uploaded presentation .js file same-origin (same reasoning as
...
export async function GET(
  ...
  const { data: file, error } = await admin.storage
    .from(PRESENT_BUCKET)
    .download(presentFilePath(id));
  if (error || !file) {
    return new Response("Not found.", { status: 404 });
  }

  // The player appends ?v=<version> (bumped on re-upload), so versioned
  // responses can be cached forever; unversioned requests stay short.
